...
/* WWhipple: BUG01053: Use + instead of & to represent 62!! */
/* Set of base64 characters, extended:
 *   A-Z a-z 0-9 + , -
 */
static const unsigned char xbase64imap_tab[128/8] = {
  0x00, 0x00, 0x00, 0x00, 0x00, 0x38, 0xff, 0x03,
  0xfe, 0xff, 0xff, 0x07, 0xfe, 0xff, 0xff, 0x07,
};

#define isxbase64imap(ch) ((ch) < 128 && ((xbase64imap_tab[(ch)>>3] >> (ch & 7)) & 1))
...
static int
utf7imap_mbtowc (conv_t conv, ucs4_t *pwc, const unsigned char *s, int n)
{
 ...
 */
static int
utf7imap_wctomb (conv_t conv, unsigned char *r, ucs4_t iwc, int n)
{
...
active:
  {
    ...
    if (isdirectimap(wc)) {
      ...
      count += ((state & 3) >= 2 ? 1 : 0) + (isxbase64imap(wc) ? 1 : 0) + 1;
      if (n < count)
        return RET_TOOSMALL;
      if ((state & 3) >= 2) {
...
static int
utf7imap_reset (conv_t conv, unsigned char *r, int n)
{
  state_t state = conv->ostate;
  if (state & 3) {
    /* deactivate base64 encoding */
